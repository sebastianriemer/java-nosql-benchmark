...
public class TestUpdateKeyValue implements TestCase {

	private DBClient dbClient;
	private static List<String> testValues;
	private static final Logger logger = Logger
			.getLogger(TestUpdateKeyValue.class);
	...
	/* constructor */
	public TestUpdateKeyValue(DBClient dbClient) {
	...
	public void execute() {
		if (dbClient != null) {
			dbClient.openConnection();
			for (int i = 0; i < testValues.size(); i++) {
				dbClient.append(Integer.toString(i), "UPD" + testValues.get(i));
			}
			dbClient.closeConnection();
		} else {
			logger.error("dbClient not set!");
